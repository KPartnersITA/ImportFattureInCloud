import os
import mysql.connector
from dotenv import load_dotenv
import fattureincloud_python_sdk
from fattureincloud_python_sdk.rest import ApiException

load_dotenv()

# --- CONFIGURAZIONE DB MYSQL ---
db_config = {
    'host': 'localhost',
    'user': os.getenv("DB_USER"),
    'password': os.getenv("DB_PASSWORD"),
    'database': os.getenv("DB_NAME")
}

# --- CONFIGURAZIONE FIC ---
configuration = fattureincloud_python_sdk.Configuration(
    host="https://api-v2.fattureincloud.it"
)
configuration.access_token = os.getenv("ACCESS_TOKEN")
company_id = int(os.getenv("COMPANY_ID"))   # ID azienda su FIC

def get_testing_clients():
    return [
        {
            "name": "Test Cliente",
            "vat_number": "00083950436",
            "address_street": "Via Test 1",
            "address_city": "Test",
            "address_province": "TA",
            "email": "test@example.com",
            "certified_email": "test@example.com",
            "phone": "1234567890",
            "tax_code": "12345678901",
            "ei_code": "7654321",
            "notes": "Test note",
            "contact_person": "Test referente",
            "code": "AG444444",
            "default_payment_method": "Bonifico Bancario vista fattura SELLA",
            
        },
       
    ]
    
# --- FUNZIONE: estrai anagrafiche dal gestionale ---
def get_clients_from_db():
    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor(dictionary=True)

    query = """
    SELECT 
account_no as code,
accountname AS name,
vacf.cf_898 AS address_street,
vacf.cf_892 AS address_zip,
vacf.cf_894 AS address_city,
vacf.cf_900 AS address_province,
vacf.cf_880 AS certified_email,
email1 AS email,
phone as phone,
vacf.cf_878 AS vat_number,
vacf.cf_1316 AS tax_code,
vacf.cf_1963 AS default_payment_method,
vacf.cf_1969 AS ei_code,
vacf.cf_1568 AS notes,
ownership AS contact_person
FROM 
vtiger_account va LEFT JOIN vtiger_accountscf vacf ON (va.accountid=vacf.accountid)
LEFT JOIN vtiger_crmentity vce ON (vce.crmid=va.accountid)
WHERE
vce.deleted=0
AND vacf.cf_878 IS NOT NULL 
AND LENGTH(vacf.cf_878) = 11 
AND va.account_type IN ("Ag. princ.","Ag. princ. collegata","Sub-A","SUB-E")
    """
    cursor.execute(query)
    results = cursor.fetchall()
    cursor.close()
    connection.close()
    return results


# --- FUNZIONE: cerca cliente per Partita IVA in FIC ---
def find_client_by_vat(api_instance, vat_number):
    try:
        resp = api_instance.list_clients(
            company_id,
            fieldset="detailed",
            q=f'vat_number="{vat_number}"',
            per_page=5
        )
        if resp.data and len(resp.data) > 0:
            return resp.data[0]   # ritorna il primo match
        return None
    except ApiException as e:
        print("Errore ricerca cliente:", e)
        return None


# --- FUNZIONE: sincronizza cliente ---
def sync_client(api_instance, client):
    """
    client = dict proveniente dal DB mysql
    """
   
    existing = find_client_by_vat(api_instance, client["vat_number"])

    # Mappatura dei campi secondo il modello SDK
    client_data = fattureincloud_python_sdk.Client(
        name=client["name"],
        address_street=client["address_street"],
        address_city=client["address_city"],
        address_province=client["address_province"],
        type="company",
        address_country="IT",
        email=client["email"],
        certified_email=client["certified_email"],
        vat_number=client["vat_number"],
        ei_code=client["ei_code"],
        phone=client["phone"],
        tax_code=client["tax_code"],
        notes=client["notes"],
        contact_person=client["contact_person"],
        code=client["code"],
        default_payment_method=client["default_payment_method"],
    )    

    try:
        if existing:  # update
            api_instance.modify_client(company_id, existing.id,
                                       fattureincloud_python_sdk.ModifyClientRequest(data=client_data))
            #print(f"✅ Aggiornato cliente: {client['name']} ({client['vat_number']})")
        else:  # create
            api_instance.create_client(company_id,
                                       fattureincloud_python_sdk.CreateClientRequest(data=client_data))
            print(f"➕ Creato nuovo cliente: {client['name']} ({client['vat_number']})")
    except ApiException as e:
        print("Errore sync cliente:", e)


# --- MAIN ---
if __name__ == "__main__":
    db_clients = get_clients_from_db()

    #db_clients = get_testing_clients()

    with fattureincloud_python_sdk.ApiClient(configuration) as api_client:
        info_api = fattureincloud_python_sdk.InfoApi(api_client)
        try:
            # Supponendo che esista un metodo per listare i metodi di pagamento...
            pm_resp = info_api.list_payment_methods(company_id)  # controlla sempre la firma del metodo!
        except AttributeError:
            print("Il metodo list_payment_methods non esiste. Controlla il nome esatto nella classe SettingsApi.")
            quit()
        
        
        api_instance = fattureincloud_python_sdk.ClientsApi(api_client)
       
        # Prima di sincronizzare i clienti, recupera i metodi di pagamento disponibili
        # Supponendo che il tuo client abbia un campo "payment_method_name", passi da nome a ID così
        name_to_id = {m.name: m.id for m in pm_resp.data}
        
        for c in db_clients:
            if c["vat_number"]:   # sincronizzo solo se ho P.IVA
                #print(c)
                method_name = c.get("default_payment_method")  # es. "Bonifico bancario"
                method_id = name_to_id.get(method_name)
                if method_id:
                    c["default_payment_method"] = fattureincloud_python_sdk.PaymentMethod(id=method_id)
                else:
                    c["default_payment_method"] = None
                    print(f"❗ Metodo di pagamento '{method_name}' non trovato per {c['name']}.")
                sync_client(api_instance, c)
            else:
                print(f"❗ Cliente {c['name']} non ha P.IVA. Non sincronizzato.")
print("Sync completato.")